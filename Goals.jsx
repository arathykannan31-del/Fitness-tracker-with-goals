...
const Goals = () => {
  ...
  const [savedGoals, setSavedGoals] = useState(() => {
    const saved = localStorage.getItem("fitness_tracked_goals");
  ...
  // Sync states to storage layers on update alterations
  useEffect(() => {
    if (savedGoals) {
      localStorage.setItem("fitness_tracked_goals", JSON.stringify(savedGoals));
    } else {
      localStorage.removeItem("fitness_tracked_goals");
    }
  }, [savedGoals]);
  ...
  const handleSubmit = (e) => {
    ...
    setSavedGoals(goals);
    setIsEditing(false);
    setGoals({ goalType: "", calorieTarget: "", stepCount: "" });
  };

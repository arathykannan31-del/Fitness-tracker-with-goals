...
function Progress() {
  ...
  const [activeMetric, setActiveMetric] = useState('burned'); // Options: 'burned' | 'eaten'

  // Extract structured chart data bundles live from Context matrices engine
  const burnedWeekData = getWeeklyChronologicalArray(workouts, 'calories');
  const eatenWeekData = getWeeklyChronologicalArray(dietLogs, 'calories');
  ...
  const maxCalories = Math.max(...currentViewDataset.data, 0);
